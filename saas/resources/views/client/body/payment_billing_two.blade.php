@php
    ...
    $user = App\Models\User::with('plan')->find($id);
    $plan = $user->plan;
    $plans = App\Models\Plan::all();

    $wordsUsed = $user->words_used;
    $wordLimit = $plan->monthly_word_limit ?? 0;
    $leftWord = max(0, $wordLimit - $wordsUsed);
...
<div class="tab-pane fade" id="payment-billing-2-tab-pane">
    ...
    <div class="pt-4">
        <h3 class="fw-normal">{{ $plan->name . ' Plan ' }}
            @if ($plan && $plan->price == 0)
                <span class="badge border border-light text-light rounded-pill ms-1">Free</span>
            @endif

        </h3>
        <div class="sub-text">{{ $wordsUsed }} words generation with access of {{ $plan->template }} templates.</div>
                ...
                <div class="pricing rounded pricing-featured bg-gradient-custom gradient-start-primary-light gradient-middle-primary-light gradient-end-blue-light gradient-angle-90">
                    <div class="pricing-content">
                        ...
                        <ul class="pricing-features">
                            ...
                            <li><em class="icon text-primary ni ni-check-circle-fill"></em><span>{{ $plan->templates }} templates for free</span></li>
                            <li><em class="icon text-primary ni ni-check-circle-fill"></em><span>No project limitation</span></li>
                            <li><em class="icon text-primary ni ni-check-circle-fill"></em><span>Multiple GPT Models</span></li>
